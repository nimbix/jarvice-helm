...
resource "google_service_account" "external_dns" {
    ...
    count = "${ lookup(var.cluster["meta"], "dns_zone_project", null) == null ? 0 : 1}"
}
...
locals {
    ...
    charts = merge(lookup(var.cluster["meta"], "dns_zone_project", null) == null ? {}:{
        "external-dns" = {
            "values" = <<EOF
image:
  registry: us.gcr.io
...
# serviceAccount:
#   annotations:
#     iam.gke.io/gcp-service-account: ${lookup(var.cluster["meta"], "dns_zone_project", null) == null ? "NoEmail":"${google_service_account.external_dns[0].email}"}
EOF
        }
},
