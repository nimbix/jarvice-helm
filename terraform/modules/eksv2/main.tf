...
resource "aws_iam_policy" "aws_ebs_csi_driver" {
    name_prefix = "${var.cluster.meta["cluster_name"]}-aws-ebs-csi-driver"
    description = "EKS aws-ebs-csi-driver policy for cluster ${module.eks.cluster_id}"
    policy = jsonencode({
        "Version": "2012-10-17",
        "Statement": [
            {
...
module "iam_assumable_role_admin_aws_ebs_csi_driver" {
    source = "terraform-aws-modules/iam/aws//modules/iam-assumable-role-with-oidc"
    version = "~> 4.7"

    create_role = true
    role_name_prefix = substr("${var.cluster.meta["cluster_name"]}-aws-ebs-csi-driver", 0, 32)
    provider_url = replace(module.eks.cluster_oidc_issuer_url, "https://", "")
