...
JARVICE provides mechanisms to limit concurrent usage of resources across global infrastructure.  These mechanisms are called *Limits*, and operate in the control plane at as "meta-scheduling".  Limits are placed on infrastructure, both on specific resources (e.g. machine types) and across all resources (e.g. concurrent CPU limits).  They apply to tenants, or teams of end-users, and to indivdual end-users.  There are two general levels of limits, both offering the same configuration capabilities:
...
Billing code|Attribute applied to payer account(s) in order to group teams or departents for accounting purposes|Attribute applied to payer account(s) for billing and invoicing outside of JARVICE
...
Limiting concurrent CPU use for the entire team while allowing specific machine types|*Team default* combination of both limit types
...
**IMPORTANT NOTE:** *mL* values apply **only** when concurrent CPU limits apply; otherwise ordinary infrastructure capacity is the final arbitrer of resource availability.  *mL* values apply in both system administrator and self-service limits which define a concurrent CPU limit.
...
2. A specific team user can be overridden to exceed the team constraints, and is evaluated separately; system administrator limits should be used to govern the payer or billing code below this to ensure maximum limits, if desired.
